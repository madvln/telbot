...
def create_poll(update: Update, context: CallbackContext, question: str, options: list, individual_user_id=None) -> None:
        ...
        if "всем" in text:
            ...
            for member in update.message.chat.members:
                if member.user.username == user_mention:
                    question = "Вы упомянули меня! Хотите продолжить?"
                    options = ["Да", "Нет"]
                    create_poll(update, context, question, options, individual_user_id=member.user.id)  # Опрос для конкретного пользователя
                    return

def welcome_handler(update: Update, context: CallbackContext) -> None:
    # Проверяем, добавлен ли бот в новый чат
    if update.message.chat.type in ['group', 'supergroup'] and update.message.new_chat_members:
...
def delete_user_message(context: CallbackContext) -> None:
    ...
    context.bot.delete_message(chat_id=context.job.context.chat.id, message_id=context.job.context.message_id)
...
def main() -> None:
    ...
    updater.dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, mention_handler))

    # Обработка добавления бота в чат
    updater.dispatcher.add_handler(MessageHandler(Filters.status_update.new_chat_members, welcome_handler))
